> [!NOTE]
>
> **Vendored copy** of [strange-v/jpegdec-esphome](https://github.com/strange-v/jpegdec-esphome) (itself based on
> [bitbank2/JPEGDEC](https://github.com/bitbank2/JPEGDEC)) with one fix: the ESP32-S3 SIMD
> preprocessor gate in jpeg.inl required CONFIG_IDF_TARGET_ESP32S3, which is never defined in that
> translation unit (sdkconfig.h is not included), so the SIMD assembly was compiled in but never
> called and all decoding ran scalar C. The gate now relies solely on JPEGDEC_HAVE_S3_SIMD, which
> this component's CMakeLists defines only for esp32s3 targets. See jpeg.inl line ~34.
