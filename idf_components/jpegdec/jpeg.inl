...
// PATCH (vendored from strange-v/jpegdec-esphome): the original condition
// also required defined(CONFIG_IDF_TARGET_ESP32S3), but that macro lives in
// sdkconfig.h, which nothing in this translation unit includes (JPEGDEC.h
// pulls in only stdlib headers under ESP_PLATFORM). The check was therefore
// always false, ESP32S3_SIMD never got defined, and every decode ran the
// scalar C path while the SIMD assembly sat unused in flash.
// JPEGDEC_HAVE_S3_SIMD is only ever defined by this component's own
// CMakeLists, and only when the IDF build target is esp32s3 — so it is both
// safe and sufficient on its own.
#if defined(ESP_PLATFORM) && defined(JPEGDEC_HAVE_S3_SIMD)
  #ifndef ESP32S3_SIMD
